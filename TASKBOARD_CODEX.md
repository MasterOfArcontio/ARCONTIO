...
- `v0.20n`: pensionamento IdleScan automatico e osservazione direzionale via Job - DONE;
- `v0.20o`: pesi intent da JSON e stati percettivi di fase nei Job - DONE;
- `v0.20q`: catalogo pesi intent dedicato e rimozione idle scan legacy - DONE.
...
Nota v0.20i: `SetFacing` e `SetNpcPos` producono dirty percettivo immediato, quindi rotazione e spostamento reale non attendono la cadenza lenta dello stato idle. `MoveToRunningActionDriver` orienta l'NPC verso la prossima cella prima del traversal multi-tick, `MovementSystem` legacy/debug mantiene lo stesso orientamento prima dello spostamento, e gli step `LookDirection` usano lo stato percettivo dedicato durante le rotazioni di osservazione.
Nota v0.20j: audit cleanup completato. Non sono state eliminate strutture percettive vive: indici persistenti, dirty, watched/observed e contatori costo sono ancora necessari. La patch ha rimosso letture ridondanti in `ObjectPerceptionSystem` e riallineato commenti interni di `World`; `MovementSystem` resta ponte tollerato.
...
Nota v0.20m: riallineamento post-closeout. `landmark_perception.period` viene eliminato e la percezione landmark e' governata solo dagli stati percettivi; la pulizia dirty passa a `PerceptionDirtyCompletionSystem`; le porte aperte/chiuse marcano dirty gli NPC nel watched cone economico; l'overlay mostra NPC percepiti su totale NPC.
Nota v0.20n: lo scan automatico idle viene pensionato come comportamento runtime autonomo. `WaitAndObserve` produce ora un job `perception.look_around.v1` composto da quattro step `LookDirection`; `SearchFood` aggiunge una fase finale di osservazione locale dopo il probe. La percezione resta centralizzata: gli step orientano l'NPC e marcano dirty, ma non eseguono scansioni fuori ciclo.
Nota v0.20o: i template job dichiarano `PerceptionState` per fase e `exitPerceptionState` per uscita finale, cosi' movimento, osservazione e ritorno a idle sono configurati in `job_templates.json`.
Nota v0.20q: i pesi degli intent sono stati spostati fuori da `game_params.json` nel file dedicato `decision_intent_score_config.json`, mantenendo lo score come unico criterio di confronto tra candidati. Il vecchio scan idle automatico, il suo stato runtime e i metodi di avvio scan legacy sono stati eliminati: l'osservazione direzionale ordinaria passa solo da job.

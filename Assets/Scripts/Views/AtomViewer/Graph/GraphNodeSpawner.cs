...
namespace SocialViewer.UI.Graph
{
    ...
    public class GraphNodeSpawner : MonoBehaviour
    {
        ...
        public void SpawnFromArcontio()
        {
            ...
            // SimulationHost.Instance pu� essere null.
            var host = SimulationHost.Instance;
            if (host == null)
            {
                Debug.LogError(
                    "[GraphNodeSpawner] SimulationHost.Instance � null.\n" +
                    "Cause pi� comune: hai premuto Play direttamente in Scene_AtomView.\n" +
            ...
            if (world == null)
            {
                Debug.LogError("[GraphNodeSpawner] host.World � null (non dovrebbe accadere).");
                return;
            }
            ...
            // ------------------------------------------------------------
            // Importante: World.NpcDna � un Dictionary<int, NpcDnaProfile>.
            // La chiave � l'ID NPC "reale" del simulatore.
            ...
            foreach (var kv in world.NpcDna)
            {
                ...
                // ------------------------------------------------------------
                // B) RectTransform (servir� per posizionamento)
                // ------------------------------------------------------------
                ...
                else
                {
                    Debug.LogWarning(
                        ...
                        "Il magnet system / collision registry non potr� usare questo nodo."
                    );
                }
                ...
                // ------------------------------------------------------------
                // E) Input forwarding (pan/zoom quando mouse � sopra la sfera)
                // ------------------------------------------------------------
                ...
                // ------------------------------------------------------------
                // I) Attiva nodo: ora OnEnable pu� registrarsi correttamente
                // ------------------------------------------------------------
                go.SetActive(true);
                iNode++;
            }

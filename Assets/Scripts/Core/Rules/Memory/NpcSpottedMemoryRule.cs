namespace Arcontio.Core
{
    ...
    /// Quando arriva un NpcSpottedEvent, crea/rafforza una traccia in memoria
    ...
    /// </summary>
    public sealed class NpcSpottedMemoryRule : IMemoryRule
    {
        public bool Matches(ISimEvent e) => e is NpcSpottedEvent;

        public bool TryEncode(World world, int observerNpcId, ISimEvent e, float witnessQuality01, out MemoryTrace trace)
        {
            trace = default;

            if (e is not NpcSpottedEvent ev)
                return false;
            ...
            {
                Type = MemoryType.NpcSpotted,       // <-- assicurati che esista nel tuo enum
                SubjectId = ev.ObservedNpcId,       // oppure ev.DefId se preferisci “conoscenza per tipo”
                CellX = ev.CellX,
                CellY = ev.CellY,
